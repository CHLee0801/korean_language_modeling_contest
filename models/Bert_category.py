...
import deepspeed
import numpy as np
from Datasets import Custom_Dataset_Category
from sklearn.metrics import f1_score, accuracy_score, precision_score, recall_score
#from kobert_transformers import get_tokenizer

class BERT_CATEGORY(pl.LightningModule):
    def __init__(self, hparams):
        super(BERT_CATEGORY, self).__init__()    
        ...
        self.category_classifier = nn.Linear(768, self.num_label)
    ...
    def forward(self, input_ids, input_mask, labels=None):
        ...
        output = self.category_classifier(output.pooler_output)
    ...
    def on_train_epoch_end(self):
        ...
        self.train_pred = []
        self.train_gt = []
        model_saved_path = self.hparams.checkpoint_path + 'category_epoch_' + str(self.epoch_num) + '.pt'
        param_dict = {}
        for name, param in self.model.state_dict().items():
            param_dict[name]=param.clone().detach().cpu()
        for name, param in self.category_classifier.state_dict().items():
            param_dict["category_classifier."+name] = param.clone().detach().cpu()
    ...
    def train_dataloader(self):
        train_dataset = Custom_Dataset_Category(self.train_path, "train", self.tokenizer, self.hparams.input_length)
        sampler = RandomSampler(train_dataset)
        dataloader = DataLoader(train_dataset,  sampler=sampler,batch_size=self.hparams.train_batch_size, drop_last=True, num_workers=self.hparams.num_workers)
        return dataloader

    def val_dataloader(self):
        validation_dataset = Custom_Dataset_Category(self.eval_path, "valid", self.tokenizer, self.hparams.input_length)
