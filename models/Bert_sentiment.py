...
import deepspeed
import numpy as np
from Datasets import Custom_Dataset_Sentiment
from sklearn.metrics import f1_score, accuracy_score, precision_score, recall_score
#from kobert_transformers import get_tokenizer

class BERT_SENTIMENT(pl.LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        self.train_pred = []
        self.train_gt = []
        model_saved_path = self.hparams.checkpoint_path + 'sentiment_epoch_' + str(self.epoch_num) + '.pt'
        param_dict = {}
    ...
    def on_validation_epoch_end(self):
        ...
        f1 = f1_score(self.val_gt, self.val_pred, average='macro')
    ...
    def train_dataloader(self):
        train_dataset = Custom_Dataset_Sentiment(self.train_path, "train", self.tokenizer, self.hparams.input_length)
        sampler = RandomSampler(train_dataset)
        dataloader = DataLoader(train_dataset,  sampler=sampler,batch_size=self.hparams.train_batch_size, drop_last=True, num_workers=self.hparams.num_workers)
        return dataloader

    def val_dataloader(self):
        validation_dataset = Custom_Dataset_Sentiment(self.eval_path, "valid", self.tokenizer, self.hparams.input_length)
